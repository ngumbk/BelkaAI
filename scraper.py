...
def main():
    ...
    index = ['photo', 'date', 'type', 'district', 'address', 'floor',
             'square_all', 'square_living', 'square_kitchen', 'comment',
             'price (thousand rubles)', 'phone', 'agency', 'email']
    rename_dic = {i:k for i, k in enumerate(index)}
    
    apartments_data = pd.DataFrame()
    print('Начинаю сбор данных. Задержка для каждой страницы - 10 сек.')
    for url in urls:
        apartments_data = parse_page(url, apartments_data)
        print(f'Собрано {len(apartments_data)} квартир')

    # Первичная обработка сырых данных
    ...
    # Сохранение исходных данных в формате csv
    apartments_data.to_csv('raw_apartments_data.csv', encoding='cp1251',
                           index=False)
